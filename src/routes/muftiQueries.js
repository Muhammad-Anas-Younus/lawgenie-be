...
import {
  createMuftiQuerySchema,
  respondMuftiQuerySchema,
} from "../validators/muftiQueryValidators.js";
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const result = await muftiQueryService.createMuftiQuery(
        req.user.id,
        req.body,
        req.file,
      );
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      const result = await muftiQueryService.respondMuftiQuery(
        req.user.id,
        req.params.id,
        req.body,
      );
